...
static AVAILABLE: u64 = 70_000_000;
static REQUIRED: u64 = 30_000_000;
...
fn calc_dir(input: &str) -> u64 {
    ...
    let used = *fs.get(&PathBuf::from("/")).unwrap();
    let unused = AVAILABLE - used;
    // minimum to delete to match the required free size
    let to_delete = REQUIRED - unused;

    let mut result = REQUIRED;

    // find the minimum folder size we can delete that does not surpass the
    // required size to be deleted.
    for v in fs.into_values() {
        if v <= REQUIRED && v >= to_delete && v < result {
            result = v;
        }
    }
